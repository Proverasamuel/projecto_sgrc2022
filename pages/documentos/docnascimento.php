<?php
  ...
  JOIN avos ON registando.idRegistando=avos.idRegistando WHERE registando.nRegisto = $id";
 
  
$dados=mysqli_query($conexao,$sqlBusca);
$linha = mysqli_fetch_assoc($dados);
if ($linha['status']=="Pendente") {
    $_SESSION['conf']="<h4 style='color: green;'>Seu documento não está pronto</h4>";
            header("Location: ../documentos/nascimento.php");
}
if ($linha['nRegisto'] != $id) {
    $_SESSION['conf']="<h4 style='color: red;'>Documento não existe</h4>";
    header("Location: ../documentos/nascimento.php");
}
  $nomeProvincia="";
 ...
 $linhaMunicipio = mysqli_fetch_assoc($sqlmuniQuery);
    ...
    JOIN registando ON registando.idUsuario=usuario.idUsuario WHERE registando.nRegisto=$id";
...
<hr>
    <div class="body-element" style="margin-left:1em;">
        ...
        <h5>Provincia: '. $linhaMunicipio['nomeProvincia'].' ***</h5>
    ...
    <div class="body-element" style="margin-left:1em;">
        ...
        <h5>Comuna: '. $linha['comunaP'].' ***</h5>
        <h5>Provincia: '. $linha['provinciaResP'].' ***</h5>
        <h5>Residencia Habitual: '. $linha['provinciaResP'].' ***</h5>
        <div style="float: right; margin-top: -8.5em; margin-right: 15em;">
            <h5>Naturalidade: '. $linha['naturalidadeP'].' ***</h5>
        <h5>Municipio: '. $linha['municipioP'].' ***</h5>
        <h5>Nacionalidade: '. $linha['nacionalidadeP']. '***</h5>
        <hr style="margin-top: -1em; color: blanchedalmond;">
        </div>
    </div>
    <hr>
    <div class="body-element" style="margin-left:1em;">
        ...
        <h5>Comuna: '. $linha['comuna'].' ***</h5>
        <h5>Provincia: '. $linha['provinciaRes'].' ***</h5>
        <h5>Residencia Habitual: '. $linha['provinciaRes'].' ***</h5>
        <div style="float: right; margin-top: -8.5em; margin-right: 15em;">
